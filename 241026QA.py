...
async def update_status():
    await client.change_presence(activity=discord.Game(name="폴 가이즈 갤러리"))
...
async def backup_thread_messages(thread):
    ...
    while not thread.archived:
        async for message in thread.history(limit=100, oldest_first=True):
            ...
            message_identifier = (timestamp, content)
            
            if message_identifier not in recorded_messages:
                with open(file_path, "a", encoding="utf-8") as f:
                    f.write(f"[{timestamp}] {nickname} ({handle}): {content}\n")
                
                recorded_messages.add(message_identifier)
