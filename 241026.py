...
async def backup_thread_messages(thread):
    ...
    recorded_message_ids = set()
    last_message = None 
    
    while not thread.archived:
        async for message in thread.history(limit=None, oldest_first=True, after=last_message):
            ...
            if message.id not in recorded_message_ids:
                with open(file_path, "a", encoding="utf-8") as f:
                    f.write(f"[{timestamp}] {nickname} ({handle}): {content}\n")
                
                recorded_message_ids.add(message.id)
